...
public class IDivInst extends Instruction {
	...
	public IDivInst(@NotNull ValueWrapper destination, @NotNull ValueWrapper source) {
		super(InstructionDef.IDIV);
	...
	public Instruction resolved(@NotNull Function<String, Long> labelResolver) {
		return new IDivInst(destination.resolved(labelResolver), source.resolved(labelResolver));
	}
	...
	@NotNull
	public static IDivInst parse(@NotNull String code) {
		String[] parts = code.split(",");
		if (parts.length != 2) {
			throw new IllegalArgumentException("Invalid number of arguments");
		}
		return new IDivInst(ValueWrapper.parse(parts[0].trim()), ValueWrapper.parse(parts[1].trim()));
	}
	
	@NotNull
	public static IDivInst load(@NotNull InstructionContext ctx) {
		ValueWrapper destination = ValueWrapper.load(ctx);
		ValueWrapper source = ValueWrapper.load(ctx);
		return new IDivInst(destination, source);
	}
