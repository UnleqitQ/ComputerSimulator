...
public class RcrInst extends Instruction {
	...
	public RcrInst(@NotNull ValueWrapper destination, @NotNull ValueWrapper source) {
		super(InstructionDef.RCR);
	...
	public void execute(@NotNull InstructionContext ctx) {
		...
		if (shift > 0) {
			...
			boolean newCarry = (destinationValue & (1L << (shift - 1))) != 0;
			ctx.registers().writeFlag(Registers.Flag.CARRY, newCarry);
			result |= oldCarry ? (1L << (destinationSize - shift)) : 0;
	...
	public Instruction resolved(@NotNull Function<String, Long> labelResolver) {
		return new RcrInst(destination.resolved(labelResolver), source.resolved(labelResolver));
	}
	...
	@NotNull
	public static RcrInst parse(@NotNull String code) {
		String[] parts = code.split(",");
		if (parts.length != 2) {
			throw new IllegalArgumentException("Invalid number of arguments");
		}
		return new RcrInst(ValueWrapper.parse(parts[0].trim()), ValueWrapper.parse(parts[1].trim()));
	}
	
	@NotNull
	public static RcrInst load(@NotNull InstructionContext ctx) {
		ValueWrapper destination = ValueWrapper.load(ctx);
		ValueWrapper source = ValueWrapper.load(ctx);
		return new RcrInst(destination, source);
	}
