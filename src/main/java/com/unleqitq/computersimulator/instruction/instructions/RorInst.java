...
public class RorInst extends Instruction {
	...
	public RorInst(@NotNull ValueWrapper destination, @NotNull ValueWrapper source) {
		super(InstructionDef.ROR);
	...
	public void execute(@NotNull InstructionContext ctx) {
		...
		long sl = destinationSize - shift;
		long result = (destinationValue >>> shift) | (destinationValue << sl);
	...
	public Instruction resolved(@NotNull Function<String, Long> labelResolver) {
		return new RorInst(destination.resolved(labelResolver), source.resolved(labelResolver));
	}
	...
	@NotNull
	public static RorInst parse(@NotNull String code) {
		String[] parts = code.split(",");
		if (parts.length != 2) {
			throw new IllegalArgumentException("Invalid number of arguments");
		}
		return new RorInst(ValueWrapper.parse(parts[0].trim()), ValueWrapper.parse(parts[1].trim()));
	}
	
	@NotNull
	public static RorInst load(@NotNull InstructionContext ctx) {
		ValueWrapper destination = ValueWrapper.load(ctx);
		ValueWrapper source = ValueWrapper.load(ctx);
		return new RorInst(destination, source);
	}
