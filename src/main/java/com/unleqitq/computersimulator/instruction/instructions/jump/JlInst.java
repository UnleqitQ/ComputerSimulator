...
public class JlInst extends Instruction {
	
	@NotNull
	private final ValueWrapper target;
	
	public JlInst(@NotNull ValueWrapper target) {
		super(InstructionDef.JL);
		this.target = target;
	}
	...
	@NotNull
	@Override
	public JlInst resolved(@NotNull Function<String, Long> labelResolver) {
		return new JlInst(target.resolved(labelResolver));
	}
	...
	@NotNull
	public static JlInst parse(@NotNull String code) {
		return new JlInst(ValueWrapper.parse(code));
	}
	
	@NotNull
	public static JlInst load(@NotNull InstructionContext ctx) {
		return new JlInst(ValueWrapper.load(ctx));
	}
