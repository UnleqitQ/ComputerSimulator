...
public class IModInst extends Instruction {
	...
	public IModInst(@NotNull ValueWrapper destination, @NotNull ValueWrapper source) {
		super(InstructionDef.IMOD);
	...
	public Instruction resolved(@NotNull Function<String, Long> labelResolver) {
		return new IModInst(destination.resolved(labelResolver), source.resolved(labelResolver));
	}
	...
	@NotNull
	public static IModInst parse(@NotNull String code) {
		String[] parts = code.split(",");
		if (parts.length != 2) {
			throw new IllegalArgumentException("Invalid number of arguments");
		}
		return new IModInst(ValueWrapper.parse(parts[0].trim()), ValueWrapper.parse(parts[1].trim()));
	}
	
	@NotNull
	public static IModInst load(@NotNull InstructionContext ctx) {
		ValueWrapper destination = ValueWrapper.load(ctx);
		ValueWrapper source = ValueWrapper.load(ctx);
		return new IModInst(destination, source);
	}
