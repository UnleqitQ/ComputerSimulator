...
public class JnzInst extends Instruction {
	
	@NotNull
	private final ValueWrapper target;
	
	public JnzInst(@NotNull ValueWrapper target) {
		super(InstructionDef.JNZ);
		this.target = target;
	}
	...
	@NotNull
	@Override
	public JnzInst resolved(@NotNull Function<String, Long> labelResolver) {
		return new JnzInst(target.resolved(labelResolver));
	}
	...
	@NotNull
	public static JnzInst parse(@NotNull String code) {
		return new JnzInst(ValueWrapper.parse(code));
	}
	
	@NotNull
	public static JnzInst load(@NotNull InstructionContext ctx) {
		return new JnzInst(ValueWrapper.load(ctx));
	}
