...
public class SarInst extends Instruction {
	...
	public SarInst(@NotNull ValueWrapper destination, @NotNull ValueWrapper source) {
	...
	public Instruction resolved(@NotNull Function<String, Long> labelResolver) {
		return new SarInst(destination.resolved(labelResolver), source.resolved(labelResolver));
	}
	...
	@NotNull
	public static SarInst parse(@NotNull String code) {
		String[] parts = code.split(",");
		if (parts.length != 2) {
			throw new IllegalArgumentException("Invalid number of arguments");
		}
		return new SarInst(ValueWrapper.parse(parts[0].trim()), ValueWrapper.parse(parts[1].trim()));
	}
	
	@NotNull
	public static SarInst load(@NotNull InstructionContext ctx) {
		ValueWrapper destination = ValueWrapper.load(ctx);
		ValueWrapper source = ValueWrapper.load(ctx);
		return new SarInst(destination, source);
	}
