...
public class AdcInst extends Instruction {
	...
	public AdcInst(@NotNull ValueWrapper destination, @NotNull ValueWrapper source) {
		super(InstructionDef.ADC);
	...
	public void execute(@NotNull InstructionContext ctx) {
		...
		long result =
			destinationValue + sourceValue + (ctx.registers().readFlag(Registers.Flag.CARRY) ? 1 : 0);
		...
		ctx.registers()
			.writeFlag(Registers.Flag.OVERFLOW, switch (destination.getSize()) {
				...
				case QWORD -> (result & 0x8000000000000000L) != (sourceValue & 0x8000000000000000L) &&
	...
	public Instruction resolved(@NotNull Function<String, Long> labelResolver) {
		return new AdcInst(destination.resolved(labelResolver), source.resolved(labelResolver));
	}
	...
	@NotNull
	public static AdcInst parse(@NotNull String code) {
		String[] parts = code.split(",");
		if (parts.length != 2) {
			throw new IllegalArgumentException("Invalid number of arguments");
		}
		return new AdcInst(ValueWrapper.parse(parts[0].trim()), ValueWrapper.parse(parts[1].trim()));
	}
	
	@NotNull
	public static AdcInst load(@NotNull InstructionContext ctx) {
		ValueWrapper destination = ValueWrapper.load(ctx);
		ValueWrapper source = ValueWrapper.load(ctx);
		return new AdcInst(destination, source);
	}
