...
public class DivInst extends Instruction {
	...
	public DivInst(@NotNull ValueWrapper destination, @NotNull ValueWrapper source) {
		super(InstructionDef.DIV);
	...
	public void execute(@NotNull InstructionContext ctx) {
		...
		long result = Long.divideUnsigned(destinationValue, sourceValue);
	...
	public Instruction resolved(@NotNull Function<String, Long> labelResolver) {
		return new DivInst(destination.resolved(labelResolver), source.resolved(labelResolver));
	}
	...
	@NotNull
	public static DivInst parse(@NotNull String code) {
		String[] parts = code.split(",");
		if (parts.length != 2) {
			throw new IllegalArgumentException("Invalid number of arguments");
		}
		return new DivInst(ValueWrapper.parse(parts[0].trim()), ValueWrapper.parse(parts[1].trim()));
	}
	
	@NotNull
	public static DivInst load(@NotNull InstructionContext ctx) {
		ValueWrapper destination = ValueWrapper.load(ctx);
		ValueWrapper source = ValueWrapper.load(ctx);
		return new DivInst(destination, source);
	}
