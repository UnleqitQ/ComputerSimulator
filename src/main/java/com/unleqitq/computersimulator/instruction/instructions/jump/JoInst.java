...
public class JoInst extends Instruction {
	
	@NotNull
	private final ValueWrapper target;
	
	public JoInst(@NotNull ValueWrapper target) {
		super(InstructionDef.JO);
		this.target = target;
	}
	...
	@NotNull
	@Override
	public JoInst resolved(@NotNull Function<String, Long> labelResolver) {
		return new JoInst(target.resolved(labelResolver));
	}
	...
	@NotNull
	public static JoInst parse(@NotNull String code) {
		return new JoInst(ValueWrapper.parse(code));
	}
	
	@NotNull
	public static JoInst load(@NotNull InstructionContext ctx) {
		return new JoInst(ValueWrapper.load(ctx));
	}
