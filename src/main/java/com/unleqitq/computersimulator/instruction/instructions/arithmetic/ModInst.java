...
public class ModInst extends Instruction {
	...
	public ModInst(@NotNull ValueWrapper destination, @NotNull ValueWrapper source) {
		super(InstructionDef.MOD);
	...
	public void execute(@NotNull InstructionContext ctx) {
		...
		long result = Long.remainderUnsigned(destinationValue, sourceValue);
	...
	public Instruction resolved(@NotNull Function<String, Long> labelResolver) {
		return new ModInst(destination.resolved(labelResolver), source.resolved(labelResolver));
	}
	...
	@NotNull
	public static ModInst parse(@NotNull String code) {
		String[] parts = code.split(",");
		if (parts.length != 2) {
			throw new IllegalArgumentException("Invalid number of arguments");
		}
		return new ModInst(ValueWrapper.parse(parts[0].trim()), ValueWrapper.parse(parts[1].trim()));
	}
	
	@NotNull
	public static ModInst load(@NotNull InstructionContext ctx) {
		ValueWrapper destination = ValueWrapper.load(ctx);
		ValueWrapper source = ValueWrapper.load(ctx);
		return new ModInst(destination, source);
	}
